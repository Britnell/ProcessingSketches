/**
 * draw image strip and it will cycle through 
 *
...
void setup()
{
  ...
  canvasSize = new PVector( 1200, 400 );
...
void send_pixels()
{
  ...
  float xStart = canvasPos.x +PIXEL_W/2 +send_index*PIXEL_W;
  float yStart = canvasPos.y +PIXEL_H/2;
  
  //for( int y=0; y<8; y++)
  for( int y=7; y>=0; y--)  // scrolling in wrong direciton... FLIP Y?
  {
    
    //for(float x=canvasPos.x+PIXEL_W/2+send_index+PIXEL_W; x<canvasPos.x+canvasSize.x;  x+=PIXEL_W )
    for( int x=0; x<8; x++)
    {
      // we on X and Y
