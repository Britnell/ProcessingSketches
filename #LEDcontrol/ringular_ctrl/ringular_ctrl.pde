/*  
 *  discrete version, finding closest LED and lighting up
...
void ring_mouse( float X, float Y , boolean chosering)
{
  ...
  if(true)
  {
    ...
    switch(mRing)
    {
      ...
      case 2:
        // 8 LEDs
        id = round(map(ang, -PI,PI, 84, 91 ));
        break;
      case 3:
        // 12 LEDs
        id = round(map(ang, -PI,PI, 72, 83 ));
        break;
      case 4:
        // 16 LEDs
        id = round(map(ang, -PI,PI, 56, 71 ));
        break;
      case 5:
        // 24 LEDs
        id = round(map(ang, -PI,PI, 32, 55 ));
        break;
      case 6:
        // 32 LEDs
        id = round(map(ang, -PI,PI, 0, 31 ));
        break;
      default:
        break;
    }
    ...
    if(id!=lastLed) {
      ...
      light_led_neighb(lastLed, color( 255, 50, 0) );
...
void light_led_neighb( int idx, color col )
{
   // loop thr pixel
    int r, g, b;
    ...
    for( int p=0; p<NMPXL; p++){
      if(p==idx){
        // color pixel
        r= int( red(col) /2+128 );
        g= int( green(col) /2+128 );
        b= int( blue(col) /2+128 );
        ardPort.write(r);
        ardPort.write(g);
        ardPort.write(b);
        ardPort.write(',');
