...
void finger_spread( Hand hand){
  ...
  if(hand.countFingers() == 5 )
  {
      ...
      if(diff > 110 || spread > 700 ) 
      {
        ...
        {
          if(ENABLE_SPREAD)  serialHID.write("#T");  // tab GUI screen
          //println("FINGER SPREAD");
          spread_at = spread;
          spread_bo=true;
          println(" # Fingers Spread at ", spread_at);
        }
      }
      ...
      if(spread_bo) 
      {
        ...
        //if(spread < 400){
        if(spread_at -spread > 90 || spread < 350 ){
          //if(ENABLE_SPREAD)  serialHID.write("#E");  // ESC
          if(ENABLE_SPREAD)  serialHID.write("#T");  // tab GUI 
          spread_bo=false;
          println("spread over ");
        }        
