...
void draw() {
  ...
  for (Hand hand : leap.getHands ()) {
    ...
    //for (Finger finger : hand.getOutstretchedFingers()  ) {
      ...
      float point_view_left = map( fingerPosition.x, 250, 900, 2.35, 2.55 );
      float point_view_right = map( fingerPosition.x, 250, 900, 0.6, 1.07 );
      ...
      float point_view_high  = map( fingerPosition.y, 250, 600, 3.3, 3.74 );
      float point_view_low  = map( fingerPosition.y, 250, 600, 2.55, 3.10 );
