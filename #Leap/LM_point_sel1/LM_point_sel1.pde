...
void draw_letters( int mid, int sel ) {
  ...
  for( int i=0; i < letters; i++ ) {
    ...
    if( i == sel) {
      ...
      fill(100, 200, 200);    strokeWeight(20);
      line( letter_pos.x + i *(step+8), letter_pos.y +30, 
             letter_pos.x + (i+1) *(step+8), letter_pos.y +30);
    }
    else {
      textSize(step *0.95);
      fill(40, 80, 120);
...
float [] y_sel_arr = new float [y_sel_size];

void check_y_sel( ) {
  PVector point = hand.getIndexFinger().getDirection();
  float y_angle = atan2(point.y, point.z);
  println(" y-angle  ", y_angle);
  
  PVector indx = hand.getIndexFinger().getPosition();
  //println(" index pos ", indx.y );
  
  // add to array
  y_sel_arr[y_sel_i] = indx.y - y_sel_last;
  
  if( y_sel_len < y_sel_size)    y_sel_len++;    // step
  y_sel_i++;    if(y_sel_i == y_sel_size)      y_sel_i = 0;    // step
  
  // calc sum
  float sum = 0;
  if(y_sel_len==y_sel_size) { 
    for( int i=0; i<y_sel_size; i++ ) {
       sum += y_sel_arr[i];
    }
    println(" sum ", sum); 
  }
